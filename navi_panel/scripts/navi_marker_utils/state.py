...
class StateManager(object):
    ...
    def update_localization(self, int_marker):
        if self._localization is None:
            ...
            self._server.erase(self._localization.name)
            self._server.insert(self._localization)

        self._localization = self._server.get(int_marker.name)
        ...
        self._server.erase(self._localization.name)
        self._server.insert(self._localization, lambda feedback: feedback)
        # DONE: validate that the color specification of the marker has been changed
