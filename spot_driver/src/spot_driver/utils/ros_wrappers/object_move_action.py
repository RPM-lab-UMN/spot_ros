...
class ObstacleMoveActionServer(ActionServerBuilder):
    ...
    def handler(self, req):
        ...
        # TODO: figure out an appropriate way to place the obstacle
        # TODO: first attempt: read the current gripper pose, and move the gripper horizontally
        # TODO: read message from the publisher
        self.ros_wrapper.logger.info("Grasp Finished! Calculating...")
        spot_curr_location = req.spot_location
        target_obstacle_location = req.obstacle_destination #IMPORTANT: thougn in this name, it's the robot's destination!!
        ...
        target_obstacle_pose.orientation.x = gripper_initial_pose.orientation.x
        target_obstacle_pose.orientation.y = gripper_initial_pose.orientation.y
        target_obstacle_pose.orientation.z = gripper_initial_pose.orientation.z
        target_obstacle_pose.orientation.w = gripper_initial_pose.orientation.w
        ...
        #self.task_wrapper._drag_arm_to(target_obstacle_pose, req.obstacle_destination.header.frame_id)
        time.sleep(2)
        ...
        # Hard-coding the original pose
        # TODO: record the original pose in odom frame; convert it into the current body frame; order 
        # the robot to go back
