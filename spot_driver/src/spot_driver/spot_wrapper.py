...
# 
# Recommendation on writing programs in this file:
#
# This is the wrapper file for short and simple tasks for SPOT, such as 
# return some data as requested, move the gripper, go down a trajectory, etc. 
# Typically, these tasks can be done easily by just sending out the commands
# with the associated clients, or reading the data acquired by the clients.
#
# should NOT call ROS servers deployed on SPOT

# Example: if you want to move the robot to a place after you study
# the point cloud, ideally you should define two functions: one to read the raw 
# point cloud data, and one to send the final motion commands using some client. 
# Other preparation tasks, such as the processing the raw data, build up the 3D scene, 
# calculate out the goal place, should NOT be defined here. You can build your own helper 
# functions here, such as transformation between frames, as long as they won't "consume" 
# too much "space". Or you may place your helper functions in other utility files. 
###############################################################################
...
class SpotWrapper:
    ...
    def arm_stow(self):
        ...
        if state.is_gripper_holding_item:
            # Open Gripper
            self._logger.info("Open the gripper")
    ...
    def gripper_open(self):
        try:
            ...
            if not success:
                ...
                # time.sleep(2.0)
                self._block_for_gripper(cmd_id, 4.0)
